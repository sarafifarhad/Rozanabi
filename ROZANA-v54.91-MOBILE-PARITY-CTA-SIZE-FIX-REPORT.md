# Rozana v54.94 — Mobile Parity and CTA Size Fix Report
...
3. **Version and cache-busting alignment**
   - Updated `package.json` to `54.94.0`.
   - Updated `package-lock.json` to `54.94.0`.
   - Updated README heading to `v54.94.0`.
   - Updated cookie consent internal version to `v54.94`.
   - Added `?v=54.94` to public CSS links so browsers do not keep serving the old mobile CSS.
...
## Validation notes
- Confirmed the landing page uses `body.landing-page-oman-body` and `header.hero.page-hero.landing-oman-hero`, so the v54.94 mobile override is scoped only to the Landing Page Design header.
- Confirmed the shared Services-page structure is not changed by the landing-specific override.
- Confirmed public HTML files now reference the v54.94 CSS query string.
